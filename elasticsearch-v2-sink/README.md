...
**Will there be data loss during migration?**
- No. The tool preserves V1 connector offsets and applies them to the V2 connector, ensuring no data is lost or duplicated.

**Can I run both V1 and V2 connectors simultaneously?**
- Yes, but this may cause data duplication. This is useful for testing but not recommended for production.
- To avoid duplication: pause the V1 connector before running the V2 connector, then delete the V1 connector once verified.

**What if my V1 connector uses multiple Elasticsearch URLs?**
...
**What happens to my SMT (Single Message Transform) configurations?**
- All `transforms.*` configurations are automatically copied to the V2 connector without modification.

**What's the difference between PRODUCTION and TEST migration modes?**
- **PRODUCTION Mode**: Migrates your connector with auto-derived settings based on your V1 configuration (recommended for final migration)
...
**Why isn't `data.stream.type: NONE` showing as a breaking change?**
- The tool automatically converts V1's `data.stream.type: NONE` to V2's default `LOGS` (when applicable). This is handled transparently during migration.

**Why isn't the default batch size difference (`2000` vs `50`) a breaking change?**
- The tool preserves your current batch size setting from the V1 connector. If you explicitly set a batch size in V1, that exact value will be used in V2.

**What happens to `behavior.on.malformed.documents: warn`?**
- V2 only supports `ignore` and `fail` values. The tool automatically converts `warn` to `ignore` during migration.

**How do I roll back if something goes wrong?**
- Since the original V1 connector is not modified by the migration tool:
  - Delete the V2 connector in Confluent Cloud
  - Resume the V1 connector
  - Your data and offsets remain intact
